    ...
    public class CharacterLinkingModule : ModuleBase<SocketCommandContext>
    {
        ...
        public async Task SetIAmAsync(ulong lodestoneId) {
            ...
            using (Context.Channel.EnterTypingState()) {
                ...
                if (await _database.UsersSettings.AnyAsync(s => s.MainCharacter == lodestoneId &&
                                                                s.VerifiedCharacter)) {
                    await ReplyAsync($"Character {profile.Character.Name} of {profile.Character.Server} is" +
                                     "already associated with and verified by another Discord user!");
                    return;
                }

                confirm = await ReplyAsync($"Found {profile.Character.Name} of {profile.Character.Server}, " +
                ...
                switch (reaction.Emote.Name) {
                    ...
                    case "\u2705": {
                        ...
                        StringBuilder message = new StringBuilder(
                            $"You will now be known as {profile.Character.Name} of {profile.Character.Server}." +
                            "To prevent another user from associating with that character");
                        if (stolen)
                            message.Append(" as you have just done to another");
                        message.Append(
                            $", add {Context.User.Id} to your lodestone profile and then run the `{prefix}verify` command.");
        ...
        public async Task SetIAmAsync([Remainder] string name) {
            ...
            using (Context.Channel.EnterTypingState()) {
                ...
                lodestoneId = profile.Character.ID;
                if (await _database.UsersSettings.AnyAsync(s => s.MainCharacter == lodestoneId &&
                                                                s.VerifiedCharacter)) {
                    await ReplyAsync($"Character {profile.Character.Name} of {profile.Character.Server} is" +
                                     "already associated with and verified by another Discord user!");
            ...
            try {
                ...
                switch (reaction.Emote.Name) {
                    ...
                    case "\u274E":
                        reacted = true;
                        await ReplyAsync("Try again with a corrected or more specific search");
                        break;
                }
            }
            finally {
